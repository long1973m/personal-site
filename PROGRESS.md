...
| 2026-08-23 | v6.10 | 笔记信息增量升级：notes.json 15/15 篇新增 point（核心增量）+value（怎么用）双字段；NoteDetail 改三层递进（说了什么→核心增量高亮→怎么用）；⌘K 搜索语料覆盖新字段 |
| 2026-08-23 | v6.11 | 笔记分类重构：数据智能(3) / AI 工程(6) / 商业观察(3) / 成长与方法(3)，替换语义混杂的旧四类；卡片配色随新分类更新 |
| 2026-08-23 | v6.14 | 个人项目区新增「个人主页」开源条目（排首位）—— 自有+源码开放+vibecoding，与 Case Study 形成区分 |
| 2026-08-23 | v6.13 | tab「旧作」更名为「个人项目」——定位修正为自有开源作品（源码可直接看），副标题同步；地铁打字冲刺年份更新为 2026 |
| 2026-08-23 | v6.12 | 新增第四个导航 tab「旧作」：安静陈列式双栏卡片（无地面反光/呼吸辉光/四角框）+ 轻量详情弹窗（只有 是什么/为什么做 两段事实层）。首条数据 past-works.json：Pi Data Agent（本地 /Users/mare/pi-agent）、地铁·打字冲刺（GitHub long1973m/metro-type-race） |
